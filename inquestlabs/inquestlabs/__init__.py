...
class inquestlabs(object):
    ...
    def set_API_key(self,key):
        ...
        Sets the api key.
        '''
        self.header = {'Authorization': f'Basic {key}'}
    ...
    def scrape_repdb_domains(self, key=None):
        ...
        return [{"ioc":item["data"] ,"type":"url"} for item in res["data"]]
    ...
    def scrape_iocdb_domains(self, choice="",key=None):
        ...
        res = json.loads(response.text)
        return [
            {"ioc": item["artifact"], "type": item["artifact_type"]}
    ...
    def request_dfi_ip(self, ip,key=None):
        ...
        '''
        url = f"https://labs.inquest.net/api/dfi/search/ioc/ip?keyword={ip}"
        response = requests.request("GET", url, headers=self.header)
        res= json.loads(response.text)
        return [hashObj["sha256"] for hashObj in res["data"]]
    ...
    def request_dfi_url(self, url,key=None):
        ...
        Returns list of hash strings
        '''
        url = f"https://labs.inquest.net/api/dfi/search/ioc/url?keyword={url}"
        response = requests.request("GET", url, headers=self.header)
        res= json.loads(response.text)
        return [hashObj["sha256"] for hashObj in res["data"]]
    ...
    def request_dfi_domain(self, domain,key=None):
        ...
        Returns list of hash strings
        '''
        url = f"https://labs.inquest.net/api/dfi/search/ioc/domain?keyword={domain}"
        response = requests.request("GET", url,headers=self.header)
        res= json.loads(response.text)
        return [hashObj["sha256"] for hashObj in res["data"]]
    ...
    def search_dfi_embedded_logic(self, search,key=None):
        ...
        '''
        url = f"https://labs.inquest.net/api/dfi/search/ext/ext_code?keyword={search}"
        response = requests.request("GET", url, headers=self.header)
        res = json.loads(response.text)
        return [item["sha256"] for item in res["data"]]
    ...
    def download_dfi_artifact_by_hash(self, sha, path=""):
        ...
        '''
        url = f"https://labs.inquest.net/api/dfi/download?sha256={sha}"
        response = requests.request("GET", url, headers=self.header)
        if response.status_code == 200:
            if (len(path) > 1):
                with open(f"{path}/{sha}", "wb+") as fh:
